...
static void ematch_process_cnstr(quant_solver_t *solver, uint32_t cidx) {
  ...
  if (nadded != 0) {
    if (oldcount == 0) {
      cnstr_learner_reset_cnstr(&solver->cnstr_learner);
    }
...
static void ematch_process_cnstr_random(quant_solver_t *solver) {
  ...
  n = qtbl->nquant;
  aux = &solver->aux_map;
  seed = uint_learner_get_seed(&solver->cnstr_learner.learner);
...
static void ematch_process_cnstr_epsilon_greedy(quant_solver_t *solver) {
  ...
  learner = &solver->cnstr_learner.learner;
...
static void ematch_process_all_cnstr(quant_solver_t *solver) {
  ...
  switch(solver->cnstr_learner.iter_mode) {
  case ITERATE_RANDOM:
    ematch_process_cnstr_random(solver);
    break;
  case ITERATE_GREEDY:
...
void init_quant_solver(quant_solver_t *solver, smt_core_t *core,
  ...
  init_cnstr_learner(&solver->cnstr_learner, &solver->qtbl);
  init_term_learner(&solver->term_learner);
...
void delete_quant_solver(quant_solver_t *solver) {
  ...
  delete_cnstr_learner(&solver->cnstr_learner);
  delete_term_learner(&solver->term_learner);
...
void quant_solver_reset(quant_solver_t *solver) {
  ...
  reset_cnstr_learner(&solver->cnstr_learner);
  reset_term_learner(&solver->term_learner);
