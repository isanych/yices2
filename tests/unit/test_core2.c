...
typedef struct core_param_s {
   ...
   * to get a MINISAT-style restart: make d_threshold very large
   * to get periodic restart: make c_factor = 1.0
   */
  uint32_t c_threshold;     // initial value of c_threshold
  uint32_t d_threshold;     // initial value of d_thershold
 ...
 */
static void sat_search(smt_core_t *core, uint32_t conflict_bound, uint32_t reduce_threshold) {
